...
/**
 * CONFIGURATION — the notes that used to be the Setups folder.
 *
 * Skills are what you load for a job. These are what is true every time,
 * whether you remember it or not. They were written for a Setups folder that
 * never opened; Setups now holds finished folders you open in Claude Code
 * (see setups.ts), so the notes are parked here, off the grid and off the
 * sitemap, with their routes intact until they get a folder of their own.
 */
export const CONFIGURATION: MaterialEntry[] = [
  {
    slug: "agents-md",
