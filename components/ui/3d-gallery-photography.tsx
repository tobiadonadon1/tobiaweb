...
interface InfiniteGalleryProps {
  ...
  /**
   * When false, the gallery does not hijack wheel / keyboard input
   * (so the host page can own scrolling). Auto-play stays on.
   */
  interactive?: boolean;
  /**
   * Lateral scatter of the planes. 1 = original diagonal spread,
   * 0 = images fly straight toward the viewer (head-on).
   */
  spread?: number;
  /** Multiplier on the auto-play forward speed (the "rush toward viewer"). */
  autoSpeed?: number;
}
...
function GalleryScene({
  ...
  // Velocity / auto-play live in refs, NOT state: they are only read inside the
  // render loop, so keeping them out of React avoids a re-render every frame
  // (which would saturate the main thread and make scroll-linked motion janky).
  ...
  const spatialPositions = useMemo(() => {
    ...
    const maxHorizontalOffset = MAX_HORIZONTAL_OFFSET * spread;
    const maxVerticalOffset = MAX_VERTICAL_OFFSET * spread;
  ...
  }, [handleWheel, handleKeyDown, interactive]);
  ...
  useFrame((state, delta) => {
    // Apply auto-play
    if (autoPlay.current) {
      scrollVelocity.current += 0.3 * delta * autoSpeed;
    }
